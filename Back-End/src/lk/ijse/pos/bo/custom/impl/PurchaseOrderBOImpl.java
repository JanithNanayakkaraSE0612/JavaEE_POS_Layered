...
public class PurchaseOrderBOImpl implements PurchaseOrderBO {
    ...
    public boolean purchaseOrder(Connection connection, OrderDTO order) throws SQLException, ClassNotFoundException {
        ...
        for (OrderDetailsDTO detailDTO : order.getOrderDetails()) {
            if (!orderDetailDAO.save(connection, new OrderDetails(detailDTO.getOrderId(), detailDTO.getItemCode(), detailDTO.getPrice(), detailDTO.getQty()))) {
                connection.rollback();
                connection.setAutoCommit(true);
                return false;
            }
